...
from archesweathersr.backbones.archesweather_layers import ICNR_init
...
class WeatherEncodeDecodeLayer(nn.Module):
    ...
    def __init__(
        ...
        if img_size == (13, 121, 240) or img_size[1:] == [121, 240]:  # 1.5deg
            self.constant_masks = torch.load(
                str(geoarches_stats_path / "archesweather_constant_masks.pt"),
                weights_only=True,
            )
        elif img_size == (13, 721, 1440) or img_size[1:] == [721, 1440]:  # 0.25deg
            self.constant_masks = torch.load(
                str(geoarches_stats_path / "era5_1440x721_constant_masks.pt"),
                weights_only=True,
            )
        else:
        ...
        self.level_padder = nn.ZeroPad3d((0, 0, 0, 0, level_pads[0], level_pads[1]))
    ...
    def encode(self, state: TensorDict, cond_state: TensorDict | None = None):
        """
        ...
        # embed
        level = state["level"]  # [bs, level_ch, Pl, Lat, Lon]
        surface = state["surface"].squeeze(-3)  # [bs, surface_ch, Lat, Lon]
        ...
        if self.constant_masks.shape[-2] % 2:
            self.constant_masks = self.constant_masks[..., :-1, :]

        constant = self.constant_masks[None, :, 0].expand(
            (bs, -1, -1, -1)
        )  # [bs, constant_dims, Lat, Lon]
    ...
    def decode(self, x):
        ...
        return TensorDict(
            {"surface": output_surface, "level": output_level},
...
class ArchesWeatherCondBackbone(nn.Module):
    ...
    def forward(self, x, cond_emb, **kwargs):
        ...
        if self.gradient_checkpointing:
            if self.first_interaction_layer:
                x = gradient_checkpoint.checkpoint(
                    self.interaction_layer, x, use_reentrant=False
                )
            x = gradient_checkpoint.checkpoint(
                self.layer1, x, cond_emb, use_reentrant=False
            )
            skip = x
            x = gradient_checkpoint.checkpoint(self.downsample, x, use_reentrant=False)
            x = gradient_checkpoint.checkpoint(
                self.layer2, x, cond_emb, use_reentrant=False
            )
            x = gradient_checkpoint.checkpoint(
                self.layer3, x, cond_emb, use_reentrant=False
            )
            x = gradient_checkpoint.checkpoint(self.upsample, x, use_reentrant=False)
            if self.use_skip and skip is not None:
                x = torch.concat([x, skip], dim=-1)
            x = gradient_checkpoint.checkpoint(
                self.layer4, x, cond_emb, use_reentrant=False
            )
        else:
            if self.first_interaction_layer:
        ...
        output = output.transpose(1, 2).reshape(
            output.shape[0], -1, self.zdim, *self.layer1_shape
        )
